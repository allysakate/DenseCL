...
def gather_tensors_batch(input_array, part_size=100, ret_rank=-1):
    ...
    all_features = []
    part_num = (
        input_array.shape[0] // part_size + 1
        if input_array.shape[0] % part_size != 0
        else input_array.shape[0] // part_size
    )
    for i in range(part_num):
        part_feat = input_array[
            i * part_size : min((i + 1) * part_size, input_array.shape[0]), ...
        ]
        assert (
            part_feat.shape[0] > 0
        ), "rank: {}, length of part features should > 0".format(rank)
    ...
    if ret_rank == -1:
        all_features = [
            np.concatenate([all_features[i][j] for i in range(part_num)], axis=0)
