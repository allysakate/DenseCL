...
class Extractor(object):
    ...
    def __init__(self, dataset, imgs_per_gpu, workers_per_gpu, dist_mode=False):
        ...
        self.data_loader = datasets.build_dataloader(
            self.dataset, imgs_per_gpu, workers_per_gpu, dist=dist_mode, shuffle=False
        )
    ...
    def __call__(self, runner):
        ...
        if self.dist_mode:
            feats = dist_forward_collect(
                func, self.data_loader, runner.rank, len(self.dataset), ret_rank=-1
            )[
                "feature"
            ]  # NxD
        else:
            feats = nondist_forward_collect(func, self.data_loader, len(self.dataset))[
                "feature"
            ]
        return feats
