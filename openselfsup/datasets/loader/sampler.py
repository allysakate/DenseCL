...
class DistributedSampler(_DistributedSampler):
    def __init__(
        self, dataset, num_replicas=None, rank=None, shuffle=True, replace=False
    ):
    ...
    def set_uniform_indices(self, labels, num_classes):
        ...
        for i, l in enumerate(images_lists):
            if len(l) == 0:
                continue
            indices.extend(
                np.random.choice(l, size_per_label, replace=(len(l) <= size_per_label))
            )
        ...
        # add extra samples to make it evenly divisible
        assert len(indices) <= self.total_size, "{} vs {}".format(
