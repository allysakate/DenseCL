...
model = dict(
    ...
    neck=dict(
        type="LinearNeck", in_channels=2048, out_channels=128, with_avg_pool=True
    ),
    head=dict(type="ContrastiveHead", temperature=0.07),
    memory_bank=dict(type="SimpleMemory", length=1281167, feat_dim=128, momentum=0.5),
)
# dataset settings
data_source_cfg = dict(
    type="ImageNet", memcached=True, mclient_path="/mnt/lustre/share/memcached_client"
)
...
# optimizer
optimizer = dict(type="SGD", lr=0.03, weight_decay=0.0001, momentum=0.9, nesterov=False)
# learning policy
