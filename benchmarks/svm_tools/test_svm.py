...
def get_chosen_costs(opts, num_classes):
    ...
    np.save(
        os.path.join(opts.output_path, "crossval_ap.npy"), np.array(train_ap_matrix)
    )
    np.save(os.path.join(opts.output_path, "chosen_cost.npy"), np.array(chosen_cost))
...
def test_svm(opts):
    ...
    for cls in range(num_classes):
        ...
        P, R, score, ap = svm_helper.get_precision_recall(eval_cls_labels, eval_preds)
        ap_matrix[cls][0] = ap
    if opts.generate_json:
