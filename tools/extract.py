...
class ExtractProcess(object):
    def __init__(
        self, pool_type="specified", backbone="resnet50", layer_indices=(0, 1, 2, 3, 4)
    ):
    ...
    def _forward_func(self, model, **x):
        ...
        feat_dict = {
            "feat{}".format(i + 1): feat.cpu() for i, feat in enumerate(flat_feats)
        }
        return feat_dict
    ...
    def extract(self, model, data_loader, distributed=False):
        ...
        if distributed:
            ...
            results = dist_forward_collect(
                func, data_loader, rank, len(data_loader.dataset)
            )
        else:
            results = nondist_forward_collect(
                func, data_loader, len(data_loader.dataset)
            )
        return results
...
def parse_args():
    ...
    parser.add_argument(
        "--work_dir", type=str, default=None, help="the dir to save logs and models"
    )
    parser.add_argument(
        "--launcher",
    ...
    parser.add_argument(
        "--port", type=int, default=29500, help='port only works when launcher=="slurm"'
    )
...
def main():
    ...
    assert (
        args.pretrained == "random" or args.checkpoint is None
    ), "Checkpoint and pretrained are exclusive."
    ...
    if args.checkpoint is not None:
        logger.info("Use checkpoint: {} to extract features".format(args.checkpoint))
        load_checkpoint(model, args.checkpoint, map_location="cpu")
    elif args.pretrained != "random":
        logger.info(
            "Use pretrained model: {} to extract features".format(args.pretrained)
        )
    else:
